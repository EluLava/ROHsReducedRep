...
mkdir -p ./TFPN_Rates_PLINK/TRUE_IBD/100GEN/bedfiles
mkdir -p ./TFPN_Rates_PLINK/TRUE_IBD/100GEN/Rates
mkdir -p ./TFPN_Rates_PLINK/WGS/bedfiles
...
WGS_bedfiles(){
    ...
    dta = read.table("./WGS_ROHs_PLINK/WGS_${SimID}_ROH.hom", header = T)

    #Extract indv
    individuals = unique(dta[,2])
    ...
    for(indiv in individuals){
        ...
        #Write the bedfile
        write.table(dta_ind, paste0("./TFPN_Rates_PLINK/WGS/bedfiles/WGS_BED_${SimID}_Ind_", indiv, ".bed"), row.names = F, quote = F, col.names = T, sep = "\t")
    }
...
R --vanilla <<EOF
    ...
    for(indiv in individuals){
        ...
        #Write the bedfile
        write.table(dta_ind, paste0("./TFPN_Rates_PLINK/TRUE_IBD/100GEN/bedfiles/TRUEIBD_BED_${sim}_Ind_", indiv, ".bed"), row.names = F, quote = F, col.names = T, sep = "\t")
    }
...
echo -e "${SimuID}\tPLINK\t${INDIV}\t${TP}\t${TN}\t${FP}\t${FN}" >> ./Analyses/WGS_PLINK_TFPN_Rates_GEN100.txt
...
echo -e "SimuID\tNB_RAD_FRAG\tINDIV\tTP\tTN\tFP\tFN" > ./Analyses/WGS_PLINK_TFPN_Rates_GEN100.txt
#Apply function to all BEDFILES
find ./TFPN_Rates_PLINK/TRUE_IBD/100GEN/bedfiles/ -name "TRUEIBD_BED_*_Ind_*.bed" | xargs -I {} bash -c 'INDIV_Rates_TRUE_IBD {}'
