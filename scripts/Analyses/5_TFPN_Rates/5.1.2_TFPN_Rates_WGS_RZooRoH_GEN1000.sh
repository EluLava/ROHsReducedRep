...
mkdir -p ./TFPN_Rates_RZooRoH/TRUE_IBD/1000GEN/bedfiles
mkdir -p ./TFPN_Rates_RZooRoH/TRUE_IBD/1000GEN/Rates
...
R --vanilla <<EOF
    dta = read.table("./TRUE_IBD/1000GEN/stats/TRUE_IBD_SEGMENTS_${sim}_GEN1000.txt", header = T)
    ...
    for(indiv in individuals){
        ...
        #Write the bedfile
        write.table(dta_ind, paste0("./TFPN_Rates_RZooRoH/TRUE_IBD/1000GEN/bedfiles/TRUEIBD_BED_${sim}_Ind_", indiv, ".bed"), row.names = F, quote = F, col.names = T, sep = "\t")
    }
...
echo -e "${SimuID}\tRZooRoH\t${INDIV}\t${TP}\t${TN}\t${FP}\t${FN}" >> ./Analyses/WGS_RZooRoH_TFPN_Rates_GEN1000.txt
...
echo -e "SimuID\tNB_RAD_FRAG\tINDIV\tTP\tTN\tFP\tFN" > ./Analyses/WGS_RZooRoH_TFPN_Rates_GEN1000.txt
#Apply function to all BEDFILES
find ./TFPN_Rates_RZooRoH/TRUE_IBD/1000GEN/bedfiles/ -name "TRUEIBD_BED_*_Ind_*.bed" | xargs -I {} bash -c 'INDIV_Rates_TRUE_IBD {}'
