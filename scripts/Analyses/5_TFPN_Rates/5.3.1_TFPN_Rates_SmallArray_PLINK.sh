...
SmallArray_bedfiles_and_TFPN_Rates_PLINK(){
    ...
    mkdir -p ./TFPN_Rates_PLINK/SmallArray/bedfiles
    mkdir -p ./TFPN_Rates_PLINK/SmallArray/Rates

    #Find the corresponding INDV file (for list of indvs)
    INDIV_FILE=$(find ./SNP_ARRAYs_PLINK/Small_ARRAY/ -name "PLINK_ROH_${SimID}_${REP}.hom.indiv")
    ...
    R --vanilla <<EOF
        ...
        for(indiv in individuals){
            ...
            #Write the bedfile
            write.table(dta_ind, paste0("./TFPN_Rates_PLINK/SmallArray/bedfiles/PLINK_SmallArray_BED_${SimID}_${REP}_Ind_", indiv, ".bed"), row.names = F, quote = F, col.names = T, sep = "\t")
...
EOF
    ...
    echo -e "SimuID\tNB_RAD_FRAG\tREP\tINDIV\tTP\tTN\tFP\tFN" > ./TFPN_Rates_PLINK/SmallArray/Rates/SmallArray_${SimID}_${REP}_TF_PN_Rates_GEN100.txt
    ...
    do
        ...
        echo -e "${SimID}\tSmallArray\t${REP}\t${INDIV}\t${TP}\t${TN}\t${FP}\t${FN}" >> ./TFPN_Rates_PLINK/SmallArray/Rates/SmallArray_${SimID}_${REP}_TF_PN_Rates_GEN100.txt
...
#List the files
FILES=./TFPN_Rates_PLINK/SmallArray/Rates/SmallArray_*_TF_PN_Rates_GEN100.txt
#Create header
echo -e "SimID\tNB_RAD_FRAG\tREP\tINDIV\tTP\tTN\tFP\tFN" > ./Analyses/SmallArray_PLINK_TFPN_Rates_GEN100.txt
#Add the files
for f in $FILES; do awk 'NR > 1 {print $0}' $f >> ./Analyses/SmallArray_PLINK_TFPN_Rates_GEN100.txt; done
