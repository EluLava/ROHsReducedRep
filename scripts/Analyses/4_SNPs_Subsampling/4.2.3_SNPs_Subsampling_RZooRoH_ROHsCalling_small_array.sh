...
SUB_ROH_CALL_FROH_ROHsDist_RZooRoH() {
	...
	bcftools view -R ./SNP_ARRAYs/Small_ARRAY/SNP_CHR_POS_${SimID}_${REP}.csv -O z -o ./SNP_ARRAYs/Small_ARRAY/VCF_SNPs_${SimID}_${REP}.vcf.gz ./Final_VCF/FINAL_${SimID}.vcf.gz

	#From VCF to OX
	plink --vcf ./SNP_ARRAYs/Small_ARRAY/VCF_SNPs_${SimID}_${REP}.vcf.gz --recode oxford --autosome --chr-set 30 --out ./SNP_ARRAYs/Small_ARRAY/TMP_${SimID}_${REP}_OX
	...
	R --vanilla <<EOF
		...
		# read the OX file
		data_Rohs = zoodata("./SNP_ARRAYs/Small_ARRAY/TMP_${SimID}_${REP}_OX.gen", zformat = "gp")

		#Create 3 HBD  and 1 non-HBD class.es model
		Mod3R <- zoomodel(K=5, krates=c(10,100,1000,10000,10000), err = 5.5e-5)
		...
		write.table(loc_table_3R, "./SNP_ARRAYs/Small_ARRAY/ROHs_SmallArray_${SimID}_${REP}.hom", quote = F, col.names = T, row.names = F)
		...
		colnames(dt_3R) <- c("Model", "SimID" , "individuals" , "FROH_SmallArray" , "REP")

		#Save the data
		write.table(dt_3R, "./SNP_ARRAYs/Small_ARRAY/FROH_RZooRoH_SmallArray_${SimID}_${REP}.txt", col.names = F , row.names = F , quote = F)


		## ROHs DIST

		#Create ROHs DIST table
		dta_dist <- cbind(rep("3R", nrow(loc_table_3R)), rep("${SimID}" , nrow(loc_table_3R)), rep("SMALL_ARRAY" , nrow(loc_table_3R)), rep("${REP}" , nrow(loc_table_3R)), loc_table_3R)
	...
	#write output
	write.table(dta_roh, file = "./SNP_ARRAYs/Small_ARRAY/ROHsDist_RZooRoH_SmallArray_${SimID}_${REP}.txt", quote = F, row.names = F, col.names = F)

EOF

	#rm TMP VCFs & OX files
	rm ./SNP_ARRAYs/Small_ARRAY/TMP_${SimID}_${REP}_OX.*
	rm ./SNP_ARRAYs/Small_ARRAY/VCF_SNPs_${SimID}_${REP}.*
...
parallel -j10 ::: SUB_ROH_CALL_FROH_ROHsDist_RZooRoH ::: $(find ./SNP_ARRAYs/Small_ARRAY/ -name "SNP_CHR_POS_*.csv")
...
#Create header
echo -e "Model\tSimID\tindividuals\tFROH_SmallArray\tREP" > ./SNP_ARRAYs/Small_ARRAY/SmallArray_RZooRoH_FROH.txt

#Loop through file to fill the data
for file in $(find ./SNP_ARRAYs/Small_ARRAY/ -name "FROH_RZooRoH_SmallArray_*.txt"); do cat ${file} >> ./SNP_ARRAYs/Small_ARRAY/SmallArray_RZooRoH_FROH.txt; done

#rm the sub files
rm ./SNP_ARRAYs/Small_ARRAY/FROH_RZooRoH_SmallArray_*.txt

#Open R for merging
R --vanilla <<EOF

	#Read Small Array + TRUE IBD data
	dtasub = read.table("./SNP_ARRAYs/Small_ARRAY/SmallArray_RZooRoH_FROH.txt", header = T)
	...
	#Save the data
	write.table(dtaFULL.1, "./Analyses/SmallArray_RZooRoH_FROH.txt", quote = F, col.names = T, row.names = F)
...
#Create header
echo -e "SimuID\tSUB_TECH\tREP\tROHs_CLASS\tMean_NROH_per_ind\tMean_Tot_Kb_per_ind" > ./Analyses/SmallArray_RZooRoH_ROHsDistributions.txt

#Loop through file to fill the data
for file in $(find ./SNP_ARRAYs/Small_ARRAY/ -name "ROHsDist_RZooRoH_SmallArray_*.txt"); do cat ${file} >> ./Analyses/SmallArray_RZooRoH_ROHsDistributions.txt; done

#rm the sub files
rm ./SNP_ARRAYs/Small_ARRAY/ROHsDist_RZooRoH_SmallArray_*.txt
