...
########################################################################################################################################################################
################################################### FILTERING ON MAF + CREATING FILES WITH ALL SNPs  ###################################################################
########################################################################################################################################################################
...
def SNP_Arraying(SimID):
    ...
    start_windows=np.linspace(start = 17501, stop = 2999942501, num = 66666)
    stop_windows=np.linspace(start = 57501, stop = 2999982501, num = 66666)
    ...
    snp_pos.to_csv('./SNP_ARRAYs/Small_ARRAY/SNP_CHR_POS_{}_{}.csv'.format(SimID, REP), index = False, header = False, sep = "\t")
...
#Launch on VCF files
parallel -j10 ::: SUBSAMPLING_ARRAY ::: $(find ./Final_VCF/ -name "FINAL_*.vcf.gz")
