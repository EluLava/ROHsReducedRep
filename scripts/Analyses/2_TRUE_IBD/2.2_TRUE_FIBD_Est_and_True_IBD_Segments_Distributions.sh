...
TRUE_IBD_F_ROHs_Dist() {
    ...
    R --vanilla <<EOF
        ...
        colnames(FtrueIBD.2) = c("SimID", "ID", "F_true_IBD_GEN100")

        #save dataframe
        write.table(FtrueIBD.2, "./TRUE_IBD/100GEN/stats/FTRUEIBD_${Sim_ID}_GEN100.txt", quote = F, col.names = F, row.names = F)
        ...
        dta_roh[,2] = rep("TRUE_IBD_100",nrow(dta_roh))
        ...
        #write output
        write.table(dta_roh, file = "./TRUE_IBD/100GEN/stats/ROHs_Dist_${Sim_ID}_GEN100.txt", quote = F, row.names = F, col.names = F)
...
for f in $(find ./TRUE_IBD/100GEN/stats/ -name "TRUE_IBD_SEGMENTS_*_GEN100.txt"); do TRUE_IBD_F_ROHs_Dist ${f}; done

# Then just merge the SIM replicates into one dataframe

## F TRUE IBD
#Create header
echo -e "SimID\tindividuals\tFroh_TRUE_IBD_100GEN" > ./TRUE_IBD/100GEN/stats/F_TRUE_IBD_100GEN.txt
#Add all files
for file in $(find ./TRUE_IBD/100GEN/stats/ -name "FTRUEIBD_*_GEN100.txt"); do cat ${file} >> ./TRUE_IBD/100GEN/stats/F_TRUE_IBD_100GEN.txt; done
#Rm per SimID file
rm ./TRUE_IBD/100GEN/stats/FTRUEIBD_*_GEN100.txt

## ROHs Dist
#Create header
echo -e "SimID\tWINDOWS.NB\tREP\tROHs_CLASS\tMean_NROH_per_ind\tMean_Tot_Kb_per_ind" > ./TRUE_IBD/100GEN/stats/TRUE_IBD_SEGMENTS_DISTRIBUTION_100GEN.txt
#Add all files
for file in $(find ./TRUE_IBD/100GEN/stats/ -name "ROHs_Dist_*_GEN100.txt"); do cat ${file} >> ./TRUE_IBD/100GEN/stats/TRUE_IBD_SEGMENTS_DISTRIBUTION_100GEN.txt; done
#Rm per SimID file
rm ./TRUE_IBD/100GEN/stats/ROHs_Dist_*_GEN100.txt
