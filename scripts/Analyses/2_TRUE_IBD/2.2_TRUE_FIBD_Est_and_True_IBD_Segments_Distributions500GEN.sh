...
TRUE_IBD_F_ROHs_Dist() {
	...
	R --vanilla <<EOF
		...
		colnames(FtrueIBD.2) = c("SimID", "ID", "F_true_IBD_GEN500")

		#save dataframe
		write.table(FtrueIBD.2, "./TRUE_IBD/500GEN/stats/FTRUEIBD_${Sim_ID}_GEN500.txt", quote = F, col.names = F, row.names = F)
		...
		#write output
		write.table(dta_roh, file = "./TRUE_IBD/500GEN/stats/ROHs_Dist_${Sim_ID}_GEN500.txt", quote = F, row.names = F, col.names = F)
...
for f in $(find ./TRUE_IBD/500GEN/stats/ -name "TRUE_IBD_SEGMENTS_*_GEN500.txt"); do TRUE_IBD_F_ROHs_Dist ${f}; done

# Then just merge the SIM replicates into one dataframe

## F TRUE IBD
#Create header
echo -e "SimID\tindividuals\tFroh_TRUE_IBD_500GEN" > ./TRUE_IBD/500GEN/stats/F_TRUE_IBD_500GEN.txt
#Add all files
for file in $(find ./TRUE_IBD/500GEN/stats/ -name "FTRUEIBD_*_GEN500.txt"); do cat ${file} >> ./TRUE_IBD/500GEN/stats/F_TRUE_IBD_500GEN.txt; done
#Rm per SimID file
rm ./TRUE_IBD/500GEN/stats/FTRUEIBD_*_GEN500.txt

## ROHs Dist
#Create header
echo -e "SimID\tWINDOWS.NB\tREP\tROHs_CLASS\tMean_NROH_per_ind\tMean_Tot_Kb_per_ind" > ./TRUE_IBD/500GEN/stats/TRUE_IBD_SEGMENTS_DISTRIBUTION_500GEN.txt
#Add all files
for file in $(find ./TRUE_IBD/500GEN/stats/ -name "ROHs_Dist_*_GEN500.txt"); do cat ${file} >> ./TRUE_IBD/500GEN/stats/TRUE_IBD_SEGMENTS_DISTRIBUTION_500GEN.txt; done
#Rm per SimID file
rm ./TRUE_IBD/500GEN/stats/ROHs_Dist_*_GEN500.txt
